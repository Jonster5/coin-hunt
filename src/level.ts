...
const randomUnit = (uv: Vec2, seed: number) =>
	new Vec2(
		Math.cos(((Math.sin(uv.dot(new Vec2(12.9898, 78.233)) + seed) * 43758.5453123) % 1) * Math.PI * 2),
		Math.sin(((Math.sin(uv.dot(new Vec2(12.9898, 78.233)) + seed) * 43758.5453123) % 1) * Math.PI * 2)
	);
...
function rotate(uv: Vec2, theta: number, noise: PerlinNoise) {
	...
	return new Vec2(
		center.x * Math.cos(theta) - center.y * Math.sin(theta),
		center.x * Math.sin(theta) + center.y * Math.cos(theta)
	);
}
	...
	for (let i = 0; i < 50; i++) {
		for (let j = 0; j < 50; j++) {
			...
			if (val < 0.33) {
				...
				const asset = assets['textures']['grass'][Math.floor(Math.random() * 10)];
				ctx.drawImage(asset, 50 * j, 50 * i, 50, 50);
